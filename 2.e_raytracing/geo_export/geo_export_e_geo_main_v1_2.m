...
loop_idy=-1;
status=0;
%function e_geo_main_v1(loop_idx,loop_idy,status)
% %% update 20161227
% %% relative repair tolerance -> automatic
% %% update v2
% %% change IAS=region-{MSCs} to first union{MSCs} then IAS=region-union

% function out = model
% path('C:\Program Files\COMSOL\COMSOL52a\Multiphysics\mli',path)
% mphstart(2036);
% filename=['log_',num2str(loop_idx),'_',num2str(loop_idy)];
% diary(filename);
%clear;%clc
...
% dse=0.6;
%load tmp_IAS2D.mat sa_msregion sa_leaf
...
model.geom('geom1').feature('del1').selection('input').set({'uni1'});
model.geom('geom1').run('del1');

model.geom('geom1').create('spl1', 'Split');
model.geom('geom1').feature('spl1').selection('input').set({'ext1(3)'});
model.geom('geom1').feature('spl1').set('keep', 'on');
model.geom('geom1').create('spl2', 'Split');
model.geom('geom1').feature('spl2').selection('input').set({'ext1(4)'});
model.geom('geom1').feature('spl2').set('keep', 'on');
model.geom('geom1').run('spl2');

ns_cell_list=cellstr(['spl1(1)';'spl1(2)';'spl2(1)';'spl2(2)'])';
%ms_cell_list=cellstr('ext1(2)');
ms_chl_list{1}=cellstr('ext2');
ms_vac_list=cellstr('ext1(1)');%treat vein as vac of bs
count_nscell=4;
count_mscell=1;
count_mschl=1;
count_msvac=1;
                                        ...
                                        if tmp_vol1>tmp_vol0
                                            ...
                                            if model.geom('geom1').obj(tmptag_mscyt).getNDomains==0
                                                ...
                                                if tmp_vol1>tmp_vol0
                                                    ...
                                                    tmptag_set=[tmptag_vac0,tmptag_chl0,tmptag_mitall_del];
                                                    model.geom('geom1').feature(tmptag_del).selection('input').set(tmptag_set);
                                                    %extend prep_CreSel
                                                    
                                                    count_mscell=count_mscell+1;
                                                    ms_cell_list(count_mscell)=cellstr(tmptag_msw1);
                                                    
                                                    count_spl=count_spl+1;flag_del_spl=1;
                                                    tmptag_spl=['spl',num2str(count_spl)];
                                                    model.geom('geom1').create(tmptag_spl, 'Split');
                                                    ...
                                                    if(tmp==1)
                                                        ...
                                                        count_dellist=count_dellist+1;
                                                        del_list(count_dellist)=cellstr(tmptag_chl1);
                                                    end
                                                    count_dellist=count_dellist+1;
                                                    del_list(count_dellist)=cellstr(tmptag_mscyt);
                                                    count_dellist=count_dellist+1;
                                                    del_list(count_dellist)=cellstr(tmptag_mituni);
                                                    count_msvac=count_msvac+1;
                                                    ms_vac_list(count_msvac)=cellstr(tmptag_vac1);
                                                end
                                            end
                                        end
                                    end
...
if model.geom('geom1').obj(tmptag_IAS).getNDomains==0
    ...
    if tmp_vol1>tmp_vol0
        ...
        model.geom('geom1').feature(tmptag_del).selection('input').set(tmptag_set);
        % model.geom('geom1').feature(tmptag_IAS).set('createselection',true);%save tag
        % tmp_str=['ias'];
        % model.geom('geom1').feature(tmptag_IAS).name(tmp_str);
        
%         %%*to match experimental measured IAS_3D
%         model.geom('geom1').measure.selection.init(3);
%         tmp_Ndomains=model.geom('geom1').obj(tmptag_IAS).getNDomains;
%         eval(['model.geom(''geom1'').measure.selection.set(tmptag_IAS,',['[',num2str(1:tmp_Ndomains),']'],');'])
%         vol_ias=model.geom('geom1').measure().getVolume();
%         IAS_3D_min=vol_ias/(vol_vein+vol_bs+vol_bse+vol_ep+vol_msregion)
%         IAS_3D_max=sa_msregion/sa_leaf
%         IAS_3D_input
%         LEAF_z
%         if IAS_3D_input<IAS_3D_min
%             display(['Error: Exact IAD_3D_input cannot be reached since it is too small. The modeled IAS_3D is ',num2str(round(IAS_3D_min,3,'significant')),' .']);
%         elseif IAS_3D_input>IAS_3D_max
%             tmp_new_LEAF_z=2*LEAF_z;
%             tmp_new_IAS_3D=(vol_ias+(tmp_new_LEAF_z-LEAF_z)*sa_msregion)/(sa_leaf*tmp_new_LEAF_z);
%             display(['Error: Exact IAD_3D_input cannot be reached since it is too big. The modeled IAS_3D is ',num2str(round(tmp_new_IAS_3D,3,'significant')),' .']);
%         else
%             tmp_new_LEAF_z=LEAF_z+((vol_vein+vol_bs+vol_bse+vol_ep+vol_msregion)*IAS_3D_input-vol_ias)/(sa_msregion-sa_leaf*IAS_3D_input);
%             tmp_new_LEAF_z
%             %%warning with too big tmp_new_LEAF_z
%             if (tmp_new_LEAF_z)/(LEAF_z)>2
%                 display(['Warning: IAS_3D_input is too much bigger than IAS_2D_input.']);
%             end
%             model.param.set('LEAF_z', [num2str(tmp_new_LEAF_z),'[m]']);
%         end
...
model.geom('geom1').feature(tmptag_del).selection('input').set({'ext1(3)' 'ext1(4)' 'ext1(6)'});
model.geom('geom1').run(tmptag_del);

%% generate geometry for ray tracing
tmptag_del=['del',num2str(count_del)];
model.geom('geom1').feature.create(tmptag_del, 'Delete');
count_del=count_del+1;
model.geom('geom1').feature(tmptag_del).selection('input').init;
del_list=[del_list,'blk2','blk4',cellstr(tmptag_uni)];
model.geom('geom1').feature(tmptag_del).selection('input').set(del_list);
model.geom('geom1').run(tmptag_del);

mphsave(model,['tmp_geo_export.mph']);

%del all except certain one; ns_cell_list, ms_cell_list, ms_chl_list, ms_vac_list
tmptag_del=['del',num2str(count_del)];
model.geom('geom1').feature.create(tmptag_del, 'Delete');
model.geom('geom1').create('csur1', 'ConvertToSurface');
alpha=0.1;
%ns
for loop_i=1:count_nscell
    model.geom('geom1').feature(tmptag_del).selection('input').init;
    clear del_list
    del_list=[ns_cell_list([1:loop_i-1,loop_i+1:count_nscell]),ms_cell_list,cat(2,ms_chl_list{:}),ms_vac_list];
    model.geom('geom1').feature(tmptag_del).selection('input').set(del_list);

    model.geom('geom1').feature('csur1').selection('input').set(ns_cell_list(loop_i));
    ...
    tri=meshdata.elem{2};
    tri=(tri+1)';
    %trisurf(tri,pts(:,1),pts(:,2),pts(:,3),'facealpha',alpha);
    %axis equal;hold on
    clear face_ms;
    for i=1:size(tri,1)
    ...
    tmp_filename=['ns',num2str(loop_i),'.ply'];
    ply_write(data,tmp_filename,'ascii','double');
end
%ms cell
for loop_i=1:count_mscell
    model.geom('geom1').feature(tmptag_del).selection('input').init;
    clear del_list
    del_list=[ns_cell_list,ms_cell_list([1:loop_i-1,loop_i+1:count_mscell]),cat(2,ms_chl_list{:}),ms_vac_list];
    model.geom('geom1').feature(tmptag_del).selection('input').set(del_list);

    model.geom('geom1').feature('csur1').selection('input').set(ms_cell_list(loop_i));
...
%     end
    ...
    data.vertex.z=pts(:,3);
    data.face.vertex_indices=face_ms;
    tmp_filename=['ms',num2str(loop_i),'.ply'];
    ply_write(data,tmp_filename,'ascii','double');
end
%ms chl
for loop_i=1:count_mscell
    for loop_j=1:count_mschl(loop_i)
        model.geom('geom1').feature(tmptag_del).selection('input').init;
        clear del_list
        ms_chl_list_del=ms_chl_list;
        tmp_list=ms_chl_list{loop_i};
        ms_chl_list_del{loop_i}=tmp_list([1:loop_j-1,loop_j+1:count_mschl(loop_i)]);
        ms_chl_list_delnew=cat(2,ms_chl_list_del{:});
        del_list=[ns_cell_list,ms_cell_list,ms_chl_list_delnew,ms_vac_list];
        model.geom('geom1').feature(tmptag_del).selection('input').set(del_list);
        
        model.geom('geom1').feature('csur1').selection('input').set(tmp_list(loop_j));
        ...
        data.vertex.z=pts(:,3);
        data.face.vertex_indices=face_ms;
        tmp_filename=['ms',num2str(loop_i),'c',num2str(loop_j),'.ply'];
        ply_write(data,tmp_filename,'ascii','double');
    end
end
%ms vac
for loop_i=1:count_msvac
    model.geom('geom1').feature(tmptag_del).selection('input').init;
    clear del_list
    del_list=[ns_cell_list,ms_cell_list,cat(2,ms_chl_list{:}),ms_vac_list([1:loop_i-1,loop_i+1:count_msvac])];
    model.geom('geom1').feature(tmptag_del).selection('input').set(del_list);

    model.geom('geom1').feature('csur1').selection('input').set(ms_vac_list(loop_i));
...
%     end
    ...
    data.vertex.z=pts(:,3);
    data.face.vertex_indices=face_ms;
    tmp_filename=['ms',num2str(loop_i),'v.ply'];
    ply_write(data,tmp_filename,'ascii','double');
end
...
%% useless code for geo_export
% % tmptag_del=['del',num2str(count_del)];
% % model.geom('geom1').feature.create(tmptag_del, 'Delete');
% % count_del=count_del+1;
% % model.geom('geom1').feature(tmptag_del).selection('input').init;
% % model.geom('geom1').feature(tmptag_del).selection('input').set(tmptag_uni);
% % model.geom('geom1').run(tmptag_del);
% 
% %% for rendering color
% % % mov vac
% % tmptag_mov='vac';%tmptag_mov=['mov',num2str(count_mov)];count_mov=count_mov+1;
% % model.geom('geom1').feature.create(tmptag_mov, 'Move');
% % model.geom('geom1').feature(tmptag_mov).selection('input').set(list_cresel(find(prep_CreSel(:,2)==4)));
% % model.geom('geom1').feature(tmptag_mov).set('disply','(EPL_thick+MST_thickatvein+EPU_thick)*1.1');
% % %model.geom('geom1').feature(tmptag_mov).set('createselection', true);
% % % mov mit
% % tmptag_mov='mit';%tmptag_mov=['mov',num2str(count_mov)];count_mov=count_mov+1;
% % model.geom('geom1').feature.create(tmptag_mov, 'Move');
% % model.geom('geom1').feature(tmptag_mov).selection('input').set(list_cresel(find(prep_CreSel(:,2)==3|prep_CreSel(:,2)==10)));
% % model.geom('geom1').feature(tmptag_mov).set('disply','(EPL_thick+MST_thickatvein+EPU_thick)*1.1');
% % %model.geom('geom1').feature(tmptag_mov).set('createselection', true);
% % % mov cyt
% % tmptag_mov='cyt';%tmptag_mov=['mov',num2str(count_mov)];count_mov=count_mov+1;
% % model.geom('geom1').feature.create(tmptag_mov, 'Move');
% % model.geom('geom1').feature(tmptag_mov).selection('input').set(list_cresel(find(prep_CreSel(:,2)==1|prep_CreSel(:,2)==8)));
% % model.geom('geom1').feature(tmptag_mov).set('disply','(EPL_thick+MST_thickatvein+EPU_thick)*1.1');
% % %model.geom('geom1').feature(tmptag_mov).set('createselection', true);
% % % mov chl
% % tmptag_mov='chl';%tmptag_mov=['mov',num2str(count_mov)];count_mov=count_mov+1;
% % model.geom('geom1').feature.create(tmptag_mov, 'Move');
% % model.geom('geom1').feature(tmptag_mov).selection('input').set(list_cresel(find(prep_CreSel(:,2)==2|prep_CreSel(:,2)==9)));
% % model.geom('geom1').feature(tmptag_mov).set('disply','(EPL_thick+MST_thickatvein+EPU_thick)*1.1');
% % %model.geom('geom1').feature(tmptag_mov).set('createselection', true);
% % % mov ep vein bse
% % tmptag_mov='leaf';%tmptag_mov=['mov',num2str(count_mov)];count_mov=count_mov+1;
% % model.geom('geom1').feature.create(tmptag_mov, 'Move');
% % model.geom('geom1').feature(tmptag_mov).selection('input').set({'ext1' 'blk2' 'blk4'});
% % model.geom('geom1').feature(tmptag_mov).set('disply','(EPL_thick+MST_thickatvein+EPU_thick)*1.1');
% % %model.geom('geom1').feature(tmptag_mov).set('createselection', true);
% 
% 
% mphsave(model,['tmp_geom_cad_nocresel_',num2str(loop_idx),'_',num2str(loop_idy),'.mph'])
% toc;
...
% mphsave(model,'tmp_geomIP_cad_nocresel.mph')
% % output tags for create selection
% if loop_idx==-1&&loop_idy==-1
%     save save_tmp0.mat -regexp '^(?!(model|ans)$).'
% end
% save list_cresel.mat list_cresel
% % catch
% %     display('FAIL again! No tmp1.mph');
% %     %%Note:   lasterr will be removed in a future version. You can obtain
% %     %%information about any error that has been generated by catching an MException.
% %     %%See Capture Information About Exceptions in the Programming Fundamentals documentation
% %     exit;
% % end
% %
