...
tmp_ab=ab_profile(1);
model.variable('var2').set('ab_bs',num2str(tmp_ab,GLB_digits));
...
for tmp_loop=1:count_chl
    ...
    model.variable(tmp_str).set('ab_1ms',num2str(tmp_ab,GLB_digits));
...
model.param.set('Xa', [num2str(0.3*fc_CA,GLB_digits),'[mol/m^3]'], 'CA concentration');
%model.param.set('Xa2', '0.5[mol/m^3]', 'CA concentration cytosol');
model.param.set('Xa2', [num2str(0.15*fc_CA,GLB_digits),'[mol/m^3]'], 'CA concentration');
...
model.param.set('dMSwall',[num2str(cellwallthick,GLB_digits),'[um]']);
model.param.set('DcMS','Dc*ptMS');
%model.param.set('ptMS','0.2','effective porosity MS wall');
model.param.set('ptMS',num2str(0.2*fc_wall_perm,GLB_digits),'effective porosity MS wall');
%model.param.set('dBSwall','0.16[um]');
model.param.set('dBSwall',[num2str(cellwallthick,GLB_digits),'[um]']);
model.param.set('DcBS','Dc*ptBS');
%model.param.set('ptBS','0.1','effective porosity BS wall');
model.param.set('ptBS',num2str(0.1*fc_wall_perm,GLB_digits),'effective porosity BS wall');
...
model.param.set('Resp', [num2str(Resp,GLB_digits),'[umol/m^2/s]'],'dark respiration in light');
...
model.param.set('Vcmax_fvcb', [num2str(Vcmax,GLB_digits),'[umol/m^2/s]']);
%model.param.set('Sc_o_fvcb', '2138', 'Sc/o (bar/bar)');
model.param.set('Sc_o_fvcb', num2str(Sco,GLB_digits), 'Sc/o (bar/bar)');
model.param.set('K_pp_fvcb', '0.5*pO2/Sc_o_fvcb');
model.param.set('K_fvcb', 'K_pp_fvcb*sc');
%model.param.set('Kc_fvcb', '23.9[Pa]*sc');
model.param.set('Kc_fvcb', [num2str(Kc/10,GLB_digits),'[Pa]*sc']);
%model.param.set('Ko_fvcb', '266[mbar]*so');
model.param.set('Ko_fvcb', [num2str(Ko/1000,GLB_digits),'[mbar]*so']);
model.param.set('Km_fvcb', 'Kc_fvcb*(1+O2/Ko_fvcb)');
%model.param.set('Jmax_fvcb', '249.7[umol/m^2/s]');
model.param.set('Jmax_fvcb', [num2str(Jm,GLB_digits),'[umol/m^2/s]']);
%model.param.set('theta_fvcb', '0.98');
model.param.set('theta_fvcb', num2str(theta,GLB_digits));
%model.param.set('s_yin', '0.572');
model.param.set('s_yin', num2str(s_yin,GLB_digits));
%model.param.set('yiill','0.653');
model.param.set('yiill',num2str(yiill,GLB_digits));
model.param.set('ab_all',num2str(ab_all,GLB_digits));
